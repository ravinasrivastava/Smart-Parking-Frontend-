...
import { useLoginphonenumberMutation } from '../../services/userAuthApi';
const Loginphonenumber = () => {
  const [server_error, setServerError] = useState({})
  const navigate = useNavigate();
  const [Loginphonenumber, { isLoading }] = useLoginphonenumberMutation()
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await Loginphonenumber(actualData)
    if (res.error) {
  ...
  return <>
      ...
      <h2>Login with Mobile Number</h2></Box>
    {server_error.non_field_errors ? console.log(server_error.non_field_errors[0]) : ""}
    {server_error.mobile ? console.log(server_error.mobile[0]) : ""}
    {server_error.password ? console.log(server_error.password[0]) : ""}
    
    <div class="card mb-3">
    <div class="row g-0 d-flex align-items-center">
      <div class="col-lg-4 d-none d-lg-flex">
        <img src="https://static3.depositphotos.com/1005574/198/v/950/depositphotos_1982796-stock-illustration-login-icon-button.jpg" alt="Trendy Pants and Shoes"
          class="w-100 rounded-t-5 rounded-tr-lg-5 rounded-bl-lg-5" />
     </div>
      <div class="col-lg-8">
        <div class="card-body py-5 px-md-5">
         <Box style={{width:"50%"}}  component='form' noValidate sx={{ mt: 1 }} id='login-form' onSubmit={handleSubmit}>
